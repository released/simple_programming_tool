...
#define BRIDGE_VCOM_PROTOCOL_UART                   1
#if BRIDGE_VCOM_PROTOCOL_UART
...
void SysTick_enable(unsigned long ticks_per_second)
{
    ...
    if (SysTick_Config(SystemCoreClock / ticks_per_second))
    {
        ...
        APP_PRINTF("Set system tick error!!\n");
        while (1);
    }
...
uint8_t check_reset_source(void)
{
    ...
    SYS->RSTSTS |= 0x1FF;
    APP_PRINTF("Reset Source <0x%08X>\r\n", src);

    #if 1   //DEBUG , list reset source
    if (src & BIT0)
    {
        APP_PRINTF("0)POR Reset Flag\r\n");       
    }
    if (src & BIT1)
    {
        APP_PRINTF("1)NRESET Pin Reset Flag\r\n");       
    }
    if (src & BIT2)
    {
        APP_PRINTF("2)WDT Reset Flag\r\n");       
    }
    if (src & BIT3)
    {
        APP_PRINTF("3)LVR Reset Flag\r\n");       
    }
    if (src & BIT4)
    {
        APP_PRINTF("4)BOD Reset Flag\r\n");       
    }
    if (src & BIT5)
    {
        APP_PRINTF("5)System Reset Flag \r\n");       
    }
    if (src & BIT6)
    {
        APP_PRINTF("6)Reserved.\r\n");       
    }
    if (src & BIT7)
    {
        APP_PRINTF("7)CPU Reset Flag\r\n");       
    }
    if (src & BIT8)
    {
        APP_PRINTF("8)CPU Lockup Reset Flag\r\n");       
    }
    #endif
    ...
    else if (src & SYS_RSTSTS_PINRF_Msk)
    {
        ...
        APP_PRINTF("power on from nRESET pin\r\n");
        return FALSE;
    } 
    else if (src & SYS_RSTSTS_WDTRF_Msk)
    {
        ...
        APP_PRINTF("power on from WDT Reset\r\n");
        return FALSE;
    }    
    else if (src & SYS_RSTSTS_LVRF_Msk)
    {
        ...
        APP_PRINTF("power on from LVR Reset\r\n");
        return FALSE;
    }    
    else if (src & SYS_RSTSTS_BODRF_Msk)
    {
        ...
        APP_PRINTF("power on from BOD Reset\r\n");
        return FALSE;
    }    
    else if (src & SYS_RSTSTS_SYSRF_Msk)
    {
        ...
        APP_PRINTF("power on from System Reset\r\n");
        return FALSE;
    } 
    else if (src & SYS_RSTSTS_CPURF_Msk)
    {
        ...
        APP_PRINTF("power on from CPU reset\r\n");
        return FALSE;         
    }    
    else if (src & SYS_RSTSTS_CPULKRF_Msk)
    {
        ...
        APP_PRINTF("power on from CPU Lockup Reset\r\n");
        return FALSE;
    }   
    
    APP_PRINTF("power on from unhandle reset source\r\n");
    return FALSE;
}
